...
> 本文件保留 T01–T18、C1–C8 和 D1–D7 的交付历史，不再作为现役任务领取入口。
> 当前使用方式以根目录 `README.md` 为准；仍待完成的评测条件只在
> `docs/comparisons/EXPANSION-STATUS.md` 维护。旧任务卡和评审记录仅作历史证据。
...
阶段 C 的 C1–C8 均已完成；表格中的空归属是历史记录，不表示任务仍可领取。
